...
from skimage.morphology import fill
...
Xs -= Xs.min()
Ys -= Ys.min()
img = np.zeros((int(np.ceil(Xs.max())) + 1, 1 + int(np.ceil(Ys.max()))))

img[np.round(Xs).astype(int), np.round(Ys).astype(int)] = 1
fimg = fill_contours_fixed(img.astype(bool))
plt.imshow(fimg)
plt.show()